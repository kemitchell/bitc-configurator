...
module.exports = function(grunt) {
	...
	grunt.initConfig({
		...
		shell: {
			...
			stopnetworking: {
				command: 'stop network-manager && echo "manual" | tee /etc/init/network-manager.override',
			},
...
function _getAptCommand() {
	return (['apt-get install -y', require('./apt-dependencies').join(' '), require('./apt-dependencies' + (_conf.isServer ? '-server' : '-client')).join(' ')].join(' '))
}
