...
* - bitc:ssh
* - bitc:setup:ssh:{client,server}
* - bitc:setup:ssh:configure
* - bitc:setup:ssh:restart
*/
...
module.exports = function(grunt) {
	...
	grunt.registerTask('bitc:setup:ssh:configure', 'set up SSH - configure daemon', function() {
		try {
			fs.renameSync('/etc/ssh/sshd_config', '/etc/bitc/ssh/sshd_config.orig');
		} catch (e) {
			grunt.log.writeln('Could not move original sshd_config (it might not have been there in the first place)'.yellow);
			// return; // Don't return, we can keep going here
		}
		...
		try {
			fs.chmodSync('/etc/ssh/sshd_config', 0644);
		} catch (e) {
			grunt.log.writeln('Could not `chmod 0644 /etc/ssh/sshd_config`'.yellow);
			// return; // Don't return, we can keep going here
		}
	...
	grunt.registerTask('bitc:setup:ssh:restart', function() {
		...
		exec('service ssh restart', {
			...
			if (error) {
				grunt.log.writeln('Failed to restart SSH daemon to reload configuration. When the setup is complete, please restart the computer.'.yellow, error.code);
				done();
				return;
			}
...
var keyUtil = {
	...
	eitherKeyExists: function(grunt) {
		return (grunt.file.isFile('/etc/bitc/ssh/globalKey') || grunt.file.isFile('/etc/bitc/ssh/globalKey.pub'));
	},
	...
	generateKeys: function(grunt, done) {
		...
		exec('ssh-keygen -q -b 2048 -t rsa -N "" -C "BITC global key" -f /etc/bitc/ssh/globalKey', {
			...
			try {
				fs.chmodSync('/etc/bitc/ssh/globalKey', 0600);
				fs.chmodSync('/etc/bitc/ssh/globalKey.pub', 0644);
			} catch (e) {
				keyUtil.deleteOldKeys(grunt);
