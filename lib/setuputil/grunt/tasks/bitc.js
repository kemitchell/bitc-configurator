...
module.exports = function(grunt) {
	...
	grunt.registerTask('bitc:fixnode:installnpm', 'install npm and n', function() {
		...
		npm.load({
			...
			npm.commands.install(['--global', '--production', 'npm', 'n@1.2.x'], function (err, data) {
				if (err) {
					grunt.fail.fatal(err);
					done(err);
					return;
