...
module.exports = function(grunt) {
	...
	grunt.registerTask('bitc:switchnetwork', 'switch to Intranet', function() {
		...
		prompt.get([
			...
			exec((_conf.isServer ? 'ifdown eth0 && ifup eth0' : 'ifdown eth0; ifup eth0'), { // On the desktop eth0 isn't up by default (it is set up by NetworkManager), so use the semicolon to ignore the error. It might exist, though, if this has already been run, so just ignore it
				timeout: 30000,
